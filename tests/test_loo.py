...
@pytest.fixture(name="radon_problematic", scope="module")
def fixture_radon_problematic(radon):
    original_loo = loo(radon, pointwise=True, var_name="y")
    original_k = original_loo.pareto_k.values
    high_k_indices = np.argsort(original_k)[-10:]

    radon_mod = radon.copy()
    y = radon_mod.observed_data.y.values.copy()

    for i, idx in enumerate(high_k_indices):
        if i % 2 == 0:
            y[idx] += 5.0
        else:
            y[idx] -= 5.0
    radon_mod.observed_data["y"] = y
    return radon_mod
